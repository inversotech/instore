...
export class TabCloseComponent implements OnInit, OnDestroy {
  ...
  public filterForm: FormGroup = this.formBuilder.group({
    ...
    id_caja: ['-1', Validators.required],
    // id_medio_pago: ['-1', Validators.required],
  ...
  private suscribeForms(): void {
    ...
    // this.filterForm.get('id_caja')?.valueChanges
    //   .subscribe(res => {
    //     setTimeout(() => {
    //       this.onBuscar();
    //     }, 0);
  ...
  public onBuscar(caja: any): void {
    ...
    this.filterForm.get('id_caja')?.setValue(caja ? caja.id_caja : '-1', { emitEvent: false });
    this.getDepositos();
  }
  ...
  public getCajaHabilitadas(): void {
    const value = this.filterForm.value;
    const params = {
      id_anho: value.id_anho,
      id_mes: value.id_mes,
    ...
    this.filterForm.get('id_caja')?.setValue('-1', { emitEvent: false });
    this.depositos = [];
    this.incomeService.getMisCajasHabilitadasAll$(params)
      .pipe(takeUntil(this.destroy$))
      .subscribe({
        next: res => {
          this.loading = false;
  ...
  get hasCajaCerrada(): boolean {
    const id_caja = this.filterForm.get('id_caja')?.value;
    if (!id_caja || id_caja === '-1') {
      return false;
    }
    const caja = this.misCajasHabilitadas.find((c: any) => c.id_caja == id_caja);
    if (!caja) {
      return false;
    }
