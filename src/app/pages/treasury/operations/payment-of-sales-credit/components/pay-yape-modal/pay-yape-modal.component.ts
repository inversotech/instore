...
@Component({
  selector: 'inverso-pay-yape-modal',
  templateUrl: './pay-yape-modal.component.html',
  styleUrls: ['./pay-yape-modal.component.scss']
})
export class PayYapeModalComponent implements OnInit, OnDestroy {
  ...
  constructor(private dialogRef: NbDialogRef<PayYapeModalComponent>,
  ...
  private buildForm() {
    const controls = {
      glosa: ['Yape, el ' + this.datePipe.transform(new Date(), 'dd/MM/yyyy HH:mm'), [Validators.required]],
    };
  ...
  public onSave() {
    ...
    if (invalid) return;
    if (!this.aData.id_caja_diario) return;
