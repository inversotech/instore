...
import { SelectUnidadesModalComponent } from '../select-unidades-modal/select-unidades-modal.component';
...
export class VentaMainComponent implements OnInit, OnDestroy {
  ...
  public articuloSelect = (value: any) => {
    this.articuloSeleccionado = value;
    this.onValidarArticulo(value);
  };
  ...
  private onValidarArticulo(articulo: any) {
    ...
    this.documentoDetalleForm.patchValue({
      ...
      tipo_gestion: articulo.tipo_gestion || 'AGREGADO',
      cantidad: articulo.tipo_gestion === 'UNIDAD' ? 0 : 1,
      unidades_seleccionadas: [],
    });

    // Si es tipo UNIDAD, abrir modal para seleccionar unidades
    if (articulo.tipo_gestion === 'UNIDAD') {
      this.abrirModalUnidades(articulo);
    }
  }

  public abrirModalUnidades(articulo: any) {
    if (!articulo) return;

    const modal = this.nbDialogService.open(SelectUnidadesModalComponent);
    modal.componentRef.instance.articulo = articulo;
    modal.componentRef.instance.idVenta = this.idVenta;

    modal.onClose.pipe(takeUntil(this.destroy$)).subscribe({
      next: (res: any) => {
        if (!res?.cancel && res?.unidades?.length > 0) {
          this.documentoDetalleForm.patchValue({
            unidades_seleccionadas: res.unidades,
            cantidad: res.unidades.length,
          });
        }
      },
      error: () => { }
  ...
  public submitDocumentoDetalle() {
    ...
    // Validar unidades para tipo UNIDAD
    if (value.tipo_gestion === 'UNIDAD') {
      if (!value.unidades_seleccionadas?.length) {
        this.nbToastrService.show(
          `Seleccione las unidades del producto.`,
          'Unidades requeridas',
          { status: "warning", icon: "alert-circle-outline" }
        );
        return;
    ...
    const data: any = {
      ...
      cantidad: value.tipo_gestion === 'UNIDAD' ? value.unidades_seleccionadas.length : value.cantidad,
      descuento_total_item: value.descuento_total_item ?? 0,
    };

    // Agregar unidades si es tipo UNIDAD
    if (value.tipo_gestion === 'UNIDAD' && value.unidades_seleccionadas?.length > 0) {
      data.ids_articulo_unidad = value.unidades_seleccionadas.map((u: any) => u.id_articulo_unidad);
    }
  ...
  public eliminarTodosDetalles() {
    if (!this.idVenta) return;

    this.salesService.deleteAllSaleDetalle$(this.idVenta)
      .pipe(takeUntil(this.destroy$))
      .subscribe({
        next: (response) => {
          this.getVentaDetalleByIdVenta();
  ...
  private limpiarFormularioDetalle() {
  ...
  public getMasters() {
    const puntoVenta$ = this.salesService.getPuntoVentaById$({ id_venta: this.idVenta }).pipe(takeUntil(this.destroy$));
    ...
    const misCajasHabilitadas$ = this.salesService.getMisCajaCajasParaVentas$({ id_venta: this.idVenta }).pipe(takeUntil(this.destroy$));
    this.loadingSpinner = true;
    forkJoin({
  ...
  public onFinalizar() {
    ...
    const params = {
      ...
      deposito_cuenta: this.hasCajaHabilitadaDeposito ? valuePay.deposito_cuenta : 0,
