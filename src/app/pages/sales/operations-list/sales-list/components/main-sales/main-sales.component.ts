...
export class MainSalesComponent implements OnInit, OnDestroy {
  ...
  public getMasters(): void {
    ...
    forkJoin({
      misPuntoVentas: this.salesService.getMisPuntoVentas$({}),
      anhos: this.anhosService.getAll$(),
      meses: this.mesesService.getAll$(),
    })
      .pipe(takeUntil(this.destroy$))
      .subscribe({
        next: ({ misPuntoVentas, anhos, meses }) => {
          this.misPuntoVentas = misPuntoVentas;
          this.anhos = anhos;
          this.meses = meses;

          if (this.misPuntoVentas.length > 0) {
            setTimeout(() => {
              this.filterForm.get('ids_punto_venta')?.setValue(this.misPuntoVentas.map((item: any) => item.id_punto_venta), { emitEvent: false });
            }, 0);
          }
  ...
  private suscribeForms(): void {
    this.filterForm.get('ids_punto_venta')?.valueChanges
      .subscribe(res => {
        setTimeout(() => {
          this.onBuscar();
        }, 100);
  ...
  private getVentas(): void {
    ...
    const params = {
      ...
      id_mes: value.id_mes,
      ids_punto_venta: value.ids_punto_venta.join(','),
    };
    this.loading = true;
