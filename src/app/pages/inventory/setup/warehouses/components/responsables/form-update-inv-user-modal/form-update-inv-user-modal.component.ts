...
export class FormUpdateInvUserModalComponent implements OnInit, OnDestroy {
    ...
    if (iitem) {
      ...
      setTimeout(() => {
        ...
        this.newForm?.patchValue({
          id_persona: this.aItem.id_persona,
  ...
  private buildForm() {
    const controls = {
      id_persona: [{ value: '', disabled: true }, [Validators.required]],
  ...
  public onSave() {
    ...
    if (confirm('¿Estás seguro de crear el item?')) {
      ...
      this.loadingSpinnerSave = true;
      this.almacenUsuariosService.updateCustom$(this.aItem.id_almacen, this.aItem.id_persona, data)
