...
export class CatalogoArticulosComponent implements OnInit, OnDestroy {
  ...
  public getAlmacenArticulos() {
    ...
    const anhoId = this.filterForm.get('id_anho')?.value || '';
    if (!anhoId) return;
  ...
  public onEliminar(item: any) {
    if (!item.id_anho || !item.id_almacen || !item.id_articulo) return;

    if (confirm('Seguro que quieres eliminar el item?')) {
      this.loadingSpinner = true;
      this.almacenArticulosService.deleteCustom$(item.id_anho, item.id_almacen, item.id_articulo)
        .pipe(
          takeUntil(this.destroy$))
        .subscribe(response => {
          this.loadingSpinner = false;
