...
export class MovementMainComponent implements OnInit, OnDestroy {
  ...
  // public onEditDetalleUnidad(item: any) {
  //   const data = {
  //     serie: item.serie,
  //     modelo: item.modelo,
  //   };
  //   this.loadingSave = true;
  //   this.movimientoDetallesService.updateUnidad$(item.id_articulo_unidad, data)
  //     .pipe(map(res => res.data), finalize(() => this.loadingSave = false))
  //     .subscribe(response => {
  //       this.getMovimientoDetalle();
  //       // this.cleanFormDetalle();
  ...
  public onSaveDetalleUnidadAll() {
    const data = {
      unidades: this.movimientoDetalles.flatMap(item => item.unidades).map(item => {
        return {
          id_articulo_unidad: item.id_articulo_unidad,
          serie: item.serie,
          modelo: item.modelo,
        };
      }),
    ...
    this.loadingSave = true;
    this.movimientoDetallesService.updateUnidadAll$(this.movimientoData.id_movimiento, data)
      .pipe(map(res => res.data), finalize(() => this.loadingSave = false))
      .subscribe(response => {
        this.getMovimientoDetalle();
        // this.cleanFormDetalle();
  ...
  public onDeleteDetalleUnidad(item: any) {
    if (!confirm('¿Seguro?')) return;

    this.loadingSave = true;
    this.movimientoDetallesService.deleteUnidad$(this.movimientoData.id_movimiento, item.id_articulo_unidad)
      .pipe(map(res => res.data), finalize(() => this.loadingSave = false))
      .subscribe(response => {
        this.getMovimientoDetalle();
        this.cleanFormDetalle();
